...
本地文件存储的部署约束与故障恢复见 [本地存储恢复](docs/storage.md)。

- 浏览器缓存比普通接口响应更新时，会自动绕过缓存再次确认当前存储的数据；同步失败时保留最近可用内容，可点击提示旁的“重新同步”恢复。
- 后台存在未保存更改时，返回首页、退出、浏览器历史导航和刷新/关闭页面会要求确认；密码草稿不会写入浏览器持久存储。
